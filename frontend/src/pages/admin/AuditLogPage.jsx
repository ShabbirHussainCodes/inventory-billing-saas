...
const ACTION_CONFIG = {
  workspace_entered: {
    ...
    icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round" className="h-3.5 w-3.5" aria-hidden="true"><path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4"/><path d="M10 17l5-5-5-5"/><path d="M15 12H3"/></svg>,
  },
  workspace_exited: {
    bg: 'bg-gray-100', color: 'text-gray-500',
    icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round" className="h-3.5 w-3.5" aria-hidden="true"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><path d="M16 17l5-5-5-5"/><path d="M21 12H9"/></svg>,
  },
  mode_switched: {
...
function DetailPill({ action, details }) {
  ...
  let text = ''
  if (action === 'mode_switched') text = `${details.from} → ${details.to}`
  else if (action === 'invoice_status_changed') text = `${details.from} → ${details.to}`
  else if (action === 'workspace_entered') text = details.mode === 'edit' ? 'Edit mode' : 'View only'
...
export default function AuditLogPage() {
  ...
  const filtered = logs.filter((log) => {
    ...
    return log.target_type === typeFilter
  })
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-gray-200 bg-white p-12 text-center">
          <p className="text-sm font-medium text-gray-900 mb-1">No logs found</p>
